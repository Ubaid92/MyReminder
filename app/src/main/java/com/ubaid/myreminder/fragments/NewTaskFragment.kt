...
class NewTaskFragment : BaseFragment(R.layout.new_task_fragment) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        reminderViewModel.editReminder.observe(viewLifecycleOwner) {
            if (it != null) {
                binding.title.text = "Edit Reminder"
                binding.btnCreateReminder.text = "Confirm modifying"
                binding.reminderNameInput.setText(it.title)
                binding.addReminderCheckbox.isChecked = it.isAlert
                binding.timePickerActions.text = DateUtils1.getFormattedTime(it.time)
                binding.datePickerActions.text = DateUtils1.getFormattedDate(it.time)
                binding.radioGroup.check(
                    when (it.priority) {
                        "High" -> binding.priorityHigh.id
                        "Medium" -> binding.priorityMedium.id
                        else -> binding.priorityLow.id
                    }
                )
            }
        }
            ...
            binding.btnCreateReminder.setOnClickListener {
                ...
                if (reminderTitle.isEmpty() || binding.datePickerActions.text == "Select date"
                    ...
                    Toast.makeText(requireContext(), "Fill all data first", Toast.LENGTH_SHORT)
                        .show()
                ...
                if (priority.isEmpty()) {
                    Toast.makeText(requireContext(), "Select priority first", Toast.LENGTH_SHORT)
                        .show()
                    return@setOnClickListener
                }
                ...
                if (reminderViewModel.editReminder.value != null) {
                    reminderData.id = reminderViewModel.editReminder.value!!.id
                    reminderViewModel.edit(reminderData)
                } else {
            ...
            val dpd = DatePickerDialog(
                ...
                { _, cYear, monthOfYear, dayOfMonth ->
                    ...
                    binding.datePickerActions.text =
                        DateUtils1.getFormattedDate(reminderViewModel.selectedTime.time.time)
                },
                year,
                month,
                day
        ...
        private fun showTimePicker() {
            ...
            val timeSetListener = TimePickerDialog.OnTimeSetListener { timePicker, hour, minute ->
                ...
                binding.timePickerActions.text =
                    DateUtils1.getFormattedTime(reminderViewModel.selectedTime.time.time)
            }
            TimePickerDialog(
                requireContext(),
                timeSetListener,
